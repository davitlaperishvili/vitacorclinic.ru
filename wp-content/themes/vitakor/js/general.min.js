jQuery(document).ready(function($) {
  $.fn.sliceHeight = function( options ){
    ...
    $(document).on('onWindowResize', function(eventdata) {
      ignoreResize.push(true);
      _slice(el);
      ignoreResize.pop();
    });
  ...
  CheckStickyFooter = function() {
    $(window).on('onWindowResize', function(eventdata){
      if(!isMobile)
      {
        try{
  ...
  function onLoadjqm(hash){
    ...
    document.addEventListener('onCompleteAction', [eventdata, $(hash.t)[0]]);
...
// Events
  ...
  $(window).scroll(function(){
    ...
    if(!ignoreScroll.length){
      ...
      timerScroll = setTimeout(function(){
        $(window).on('onWindowScroll', false);
      }, 100);
    }
  ...
  document.addEventListener('onWindowScroll', function(eventdata) {
    try{
  ...
  $(document).on('onSliderStart', function(eventdata) {
    console.log(12)
    try{
      ignoreResize.push(true);
      if(eventdata){
  ...
  document.addEventListener('onSubmitForm', function(eventdata){
    try{
  ...
  $(window).resize(function(){
    ...
    if(!ignoreResize.length){
      ...
      timerResize = setTimeout(function(){
        $(window).on('onWindowResize', false);
      }, 100);
    }
    documentScrollTopLast = $(document).scrollTop();
  });
  ...
  InitFlexSliderClass = function(class_name) {
    ...
    config.prevText = "FANCY_PREV",           //String: Set the text for the "previous" directionNav item
      ...
      config.after = config.start = function(slider){
        ...
        document.addEventListener('onSlide', [eventdata]);
      }

    config.end = function(slider){
      var eventdata = {slider: slider};
      document.addEventListener('onSlideEnd', [eventdata]);
    }
    ...
    $('.flexslider:not(.thmb):visible').each(function(){
      ...
      config.prevText = "FANCY_PREV",           //String: Set the text for the "previous" directionNav item
        ...
        config.start = function(slider){
          var eventdata = {slider: slider};
          document.addEventListener('onSliderStart', [eventdata]);
          document.addEventListener('onSlide', [eventdata]);
        }
      ...
      config.before = function(slider){
        var eventdata = {slider: slider};
        document.addEventListener('onSlideBefore', [eventdata]);
      }
  ...
  $(document).ready(function(){
    ...
    document.addEventListener('onCompleteAction', function(eventdata, _this){
      try{
    ...
    document.addEventListener('onCounterGoals', function(eventdata){
      ...
      if(!th.hasClass('loading'))
      {
        ...
        $.ajax({
          ...
          success: function(html){
            ...
            document.addEventListener('onCompleteAction', [eventdata, th[0]]);
      ...
      if(!$this.hasClass('loadings')){
        ...
        $.ajax({
          ...
          success: function(html){
            ...
            var eventdata = {action:'ajaxContentLoaded'};
            document.addEventListener('onCompleteAction', [eventdata, $(this).find('.more_text_ajax')[0]]);
            document.addEventListener('onCompleteActionComponent', [eventdata, $(this).find('.more_text_ajax')[0]]);
    ...
    if($('.styled-block .row > div.col-md-3').length){
      document.addEventListener('onWindowResize', function(eventdata) {
        try{
          ignoreResize.push(true);
    ...
    {
      document.addEventListener('onCompleteAction', function(eventdata){
    ...
    $(document).on('click', '.to_cart', function(e){
      ...
      if(!isNaN(itemData.ID) && parseInt(itemData.ID) > 0){
        ...
        $.ajax({
          ...
          var eventdata = {action:'loadForm'};
          document.addEventListener('onCompleteAction', [eventdata, _this[0]]);

          if(bBasketTop){
            $('.ajax_basket').html(html);
            var eventdata = {action:'loadBasket'};
            document.addEventListener('onCompleteAction', [eventdata, $('.ajax_basket')[0]]);
          }
          ...
          if(arMedc2Options['THEME']['USE_SALE_GOALS'] != 'N')
          {
            ...
            document.addEventListener('onCounterGoals', [eventdata]);
          }
        });
      }
      else{
  ...
  window.addEventListener('onWindowResize', function(eventdata) {
    try{
      ignoreResize.push(true);
  ...
  window.addEventListener('onSlide', function(eventdata) {
    try{
      ignoreResize.push(true);
      if(eventdata){
        var slider = eventdata.slider;
  ...
  window.addEventListener('onSlideEnd', function(eventdata) {
    try{
      ignoreResize.push(true);
      if(eventdata){
        var slider = eventdata.slider;
...
return;*/
    var node = jQuery("BX_YMAP_MAP_v33");
    node.innerHTML = '';
    map = window.GLOBAL_arMapObjects['MAP_v33'] = new ymaps.Map(node, {
      center: [55.424367818435, 37.547154095481],
      zoom: 17,
      type: 'yandex#map',
    ...
    map.geoObjects.events.add('balloonclose', function (e) {
      setTimeout(function () {
        jQuery('.ymaps-image-with-content').each(function () {
          if (!jQuery(this).find('.marker').length) {
            jQuery(this).prepend('<div class="marker">' + markerSVG + '</div>');
          }
        });
      }, 20);
    });
    map.events.add('boundschange', function (e) {
      //jQuery('.ymaps-image-with-content .marker').remove();
      setTimeout(function () {
        jQuery('.ymaps-image-with-content').each(function () {
          if (!jQuery(this).find('.marker').length) {
            jQuery(this).prepend('<div class="marker">' + markerSVG + '</div>');
...
*/
  ...
  var markerSVG = '<svg xmlns="http://www.w3.org/2000/svg" width="46" height="57" viewBox="0 0 46 57">'
    ...
    +'</svg>';
  var clusterSVG = '<div class="cluster_custom"><span>jQuery[properties.geoObjects.length]</span>'
...
*/
    ...
    arObjects.PLACEMARKS[arObjects.PLACEMARKS.length] = BX_YMapAddPlacemark(map, {'LON':'37.547309342781','LAT':'55.424538734657','TEXT':'Многопрофильный медицинский центр \"Витакор\"'});

    jQuery('.contacts.front.type_2 .detail_desc_items .top-close').on('click', function(){
  ...
  function showContactsType(){
    if(jQuery.cookie('SHOW_ON_MAP') == 'Y'){
      jQuery('.contacts_wrap input.switcher').prop('checked', true);
      jQuery('.contacts_wrap .show_list').removeClass('active');
      jQuery('.contacts_wrap .show_map').addClass('active');
      jQuery('.map.front').removeClass('hidden');
      jQuery('.contacts_wrap .contacts:not(.map_wrap)').addClass('hidden');
      jQuery('.contacts_wrap .contacts:not(.map_wrap)').closest('.contacts_wrap').removeClass('border-bottom');
      setTimeout(function(){
        jQuery('.contacts_wrap').find('.item').css('opacity', 0);
      }, 100);
    }
    else{
      jQuery('.contacts_wrap input.switcher').prop('checked', false);
      jQuery('.contacts_wrap .show_list').addClass('active');
      jQuery('.contacts_wrap .show_map').removeClass('active');
      jQuery('.map.front').addClass('hidden');
      jQuery('.contacts_wrap .contacts:not(.contacts.map_wrap)').removeClass('hidden');
      jQuery('.contacts_wrap .contacts:not(.map_wrap)').closest('.contacts_wrap').addClass('border-bottom');
      jQuery('.map.front .bx-yandex-view-layout').css('opacity', 0);
      jQuery(window).resize();
      setTimeout(function(){
        jQuery('.contacts_wrap').find('.item').css('opacity', 1);
      }, 200);
    }
  }
  jQuery(document).ready(function(){
    var mapScale = {'yandex_lat':'55.424367818435','yandex_lon':'37.547154095481','yandex_scale':'17','PLACEMARKS':[{'LON':'37.547309342781','LAT':'55.424538734657','TEXT':'Многопрофильный медицинский центр \"Витакор\"'}]};
    setTimeout(function(){
      jQuery('.contacts.front .bx-yandex-map').css('opacity', 1);
    }, 1100);
    jQuery('.contacts-stores .item .top-wrap .show_on_map>span').on('click', function(){
      var arCoordinates = jQuery(this).data('coordinates').split(','),
        mapOffsetTop = jQuery('.contacts-page-map').offset().top;
      jQuery('html, body').animate({scrollTop: mapOffsetTop - (isMobile ? 20 : 180)}, 300);
      map.setCenter([arCoordinates[0], arCoordinates[1]], '17');
    });

    var a = new DG.OnOffSwitchAuto({
      cls:'.switcher',
      textOn:"",
      heightTrack:11,
      textOff:"",
      ...
      listener: function(name, checked){
        if(checked){
          console.log('fuck')
          jQuery.cookie('SHOW_ON_MAP', 'Y', {path: arMedc2Options['SITE_DIR'], expires: 365});
          setTimeout(function(){
            geo_result.applyBoundsToMap(window.map);
            jQuery('.map.front .bx-yandex-view-layout').css('opacity', 1);
          ...
          jQuery.cookie('SHOW_ON_MAP', 'N', {path: arMedc2Options['SITE_DIR'], expires: 365});
        }
        showContactsType();
      }
    });
    showContactsType();
    jQuery('.contacts.front.type_2 .items .item').on('click', function(){
      var arCoordinates = jQuery(this).data('coordinates').split(',');
      map.setCenter([arCoordinates[0], arCoordinates[1]], '17');
    });
  });
