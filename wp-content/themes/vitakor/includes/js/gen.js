jQuery(document).ready(function ($) {
  var arMedc2Options = {
    ...
    THEME: {
      ...
      SOCIAL_FACEBOOK:
        "http://www.facebook.com/Клиника-Витакор-Подольск-107292814746347/",
      SOCIAL_TWITTER: "",
      SOCIAL_YOUTUBE: "",
  ...
  jQuery(document).on("change", ".uploader input[type=file]", function () {
    if (
      ...
      !jQuery(this).next().hasClass("resetfile")
    ) {
      jQuery(
  ...
  jQuery(document).on(
    "click",
    ".uploader input[type=file] + .resetfile",
    function () {
  ...
  CheckTopMenuDotted = function () {
    ...
    if (menu.length) {
      menu
              ...
              if (!menuItemOldSave.length) {
                ...
                if (hideWidth > block_w) {
                  ...
                  menuItemNewSave =
                    ...
                    "</div></td>";
                  menuItemOldSave.remove();
                  jQuery(menuItemNewSave).insertBefore(
                    jQuery(this).find("td.js-dropdown")
                  );
        ...
        .promise()
        .done(function () {
          jQuery(".menu-row.bgcolored .mega-menu .table-menu").css(
            "overflow",
            "visible"
          );
        });
  ...
  InitTopestMenuGummi = function () {
    ...
    function _gummi() {
      ...
      var arItems = $menuTopest.find(
        ">div:not(.more),.more>.dropdown>.wrap>div"
      );
      var cntItems = arItems.length;
      if (cntItems) {
        var bMore = false;
      ...
      if (!isMobile) {
        try {
          ...
          var bodyHeight = jQuery(".body").outerHeight(true, true),
            ...
            topHeaderHeight = jQuery(".top-block").length
              ? jQuery(".top-block").outerHeight(true, true)
              : 0,
            panelHeight = jQuery("#panel").length
              ? jQuery("#panel").outerHeight(true, true)
              : 0,
        ...
        jQuery(document).on("scroll", function () {
          if (!isMobile) {
            ...
            headerCanFix =
              !mobileBtnMenu.is(
                ":visible"
              ) /* && !jQuery('.dropdown-menu:visible').length*/;

            var current_is = jQuery(".search-wrapper .search-input:visible"),
              title_search_result = jQuery(
    ...
    if (mfixed.length && isMobile) {
      ...
      jQuery(window).scroll(function () {
        ...
        if (window.matchMedia("(max-width:991px)").matches) {
          if (
            jQuery("body.mfixed_Y.mfixed_view_scroll_top #mobileheader")
              .length ||
            jQuery("body.mfixed_Y.mfixed_view_scroll_top #mobileheadersimple")
              .length
          ) {
  ...
  CheckObjectsSizes = function () {
    jQuery(".container iframe,.container object,.container video").each(
      function () {
        ...
        if (height_attr && width_attr) {
          jQuery(this).css(
            "height",
            (jQuery(this).outerWidth() * height_attr) / width_attr
          );
        }
      }
    );
    ...
    if (arMedc2Options["THEME"]["SCROLLTOTOP_TYPE"] !== "NONE") {
      ...
      jQuery("body").append(
        jQuery("<a />")
          .addClass(
            "scroll-to-top " +
              arMedc2Options["THEME"]["SCROLLTOTOP_TYPE"] +
              " " +
              arMedc2Options["THEME"]["SCROLLTOTOP_POSITION"]
          )
          .attr({
            href: "#",
            id: "scrollToTop",
                ...
                if (bVideoPlayerYoutube) {
                  waitYTPlayer(150, function () {
                    ...
                    if (bVideoSoundDisabled) {
                      window[players[videoID].id].addEventListener(
                        "onReady",
                        muteYoutubePlayer
                      );
                    }
          ...
          $slides.each(function (i, node) {
            ...
            $_slide.prepend(
              '<video id="player_' +
                ...
                (bVideoCover == true ? " cover" : "") +
                '"' +
                (bVideoLoop == true ? " loop " : "") +
                (bVideoSoundDisabled == true ? " muted " : "") +
                ' autoplay><source src="' +
                videoPlayerSrc +
            ...
            if (typeof players !== "undefined" && players) {
              ...
              document
                ...
                .addEventListener("play", onHtml5PlayerPlay);
              // }
            }
          });
        }
    ...
    var maxHeight = this.map(function (i, e) {
      ...
      if (classNull !== false) {
        minus_height = parseInt(
          jQuery(e).find(classNull).actual("outerHeight")
        );
      }
      ...
      if (
        ...
        var elw =
          typeof options.item !== "undefined" && options.item.length
            ? jQuery(options.item).first().getFloatWidth()
      ...
      if (
        ...
        if (options.slice) {
          for (var i = 0; i < el.length; i += options.slice) {
            jQuery(el.slice(i, i + options.slice)).equalizeHeights(
              options.outer,
              options.classNull,
              options.minHeight,
              options.typeResize,
        ...
        if (options.lineheight) {
          ...
          el.each(function () {
            jQuery(this).css(
              "line-height",
              jQuery(this).actual("height") + lineheightAdd + "px"
            );
          });
        }
      }
  ...
  function onLoadjqm(hash) {
    var name = jQuery(hash.t).data("name"),
      top =
        (jQuery(window).height() > hash.w.height()
          ? Math.floor((jQuery(window).height() - hash.w.height()) / 2)
          : 0) + "px";
    ...
    jQuery.each(jQuery(hash.t).get(0).attributes, function (index, attr) {
      if (/^data\-autoload\-(.+)jQuery/.test(attr.nodeName)) {
        ...
        var el = jQuery('[name="' + key.toUpperCase() + '"]').length
          ? jQuery('[name="' + key.toUpperCase() + '"]')
          : jQuery('.form-group [data-sid="' + key.toUpperCase() + '"]'); //jQuery('[name="'+key.toUpperCase()+'"]');
        //console.log(el);
        ...
        if (
          typeof el[0] !== "undefined" &&
          typeof el[0].tagName !== "undefined" &&
          el[0].tagName === "SELECT"
        ) {
            ...
            jQuery(el)
              ...
              .each(function () {
                var dataID = jQuery(this).data("id").toString(),
                  arDataID =
                    dataID.indexOf(",") !== -1 ? dataID.split(",") : [dataID];
    ...
    if (name === "order_product") {
      if (jQuery(hash.t).data("product")) {
        ...
        jQuery('input[name="PRODUCT"]')
          .val(jQuery(hash.t).data("product"))
          .attr("readonly", "readonly")
          .attr("title", jQuery('input[name="PRODUCT"]').val());
      }
    }
    if (name === "question") {
      if (jQuery(hash.t).data("product")) {
        jQuery('input[name="NEED_PRODUCT"]')
          .closest(".form-group")
          .addClass("input-filed");
        jQuery('input[name="NEED_PRODUCT"]')
          .val(jQuery(hash.t).data("product"))
          .attr("readonly", "readonly")
          .attr("title", jQuery('input[name="NEED_PRODUCT"]').val());
      }
    }
    hash.w
      .addClass("show")
  ...
  jQuery.fn.jqmEx = function () {
    jQuery(this).each(function () {
      ...
      if (name.length) {
        ...
        if (
          !jQuery("." + name + '_frame[data-trigger="' + encTriggerAttrs + '"]')
            .length
        ) {
          if (_this.attr("disabled") != "disabled") {
            ...
            jQuery("body").append(
              '<div class="' +
                name +
                '_frame jqmWindow" style="width:560px" data-trigger="' +
                encTriggerAttrs +
                '"></div>'
            );
            jQuery(
  ...
  CheckHeaderFixedMenu = function () {
    if (
      arMedc2Options["THEME"]["HEADER_FIXED"] === 2 &&
      jQuery("#headerfixed .js-nav").length &&
      ...
      var all_width = 0,
        ...
        padding_menu =
          jQuery("#headerfixed .logo-row.v2 .menu-block").actual("outerWidth") -
          jQuery("#headerfixed .logo-row.v2 .menu-block").actual("width");
      jQuery("#headerfixed .logo-row.v2 > .inner-table-block").each(
        function () {
      ...
      jQuery("#headerfixed .js-nav").width(
        cont_width - all_width - padding_menu
      );
    }
  };
  ...
  CheckTopMenuPadding = function () {
    if (
      jQuery(".logo_and_menu-row .right-icons .wrap_icon").length &&
      jQuery(".logo_and_menu-row .menu-row").length
    ) {
      ...
      jQuery(".logo_and_menu-row .menu-row>div")
        .each(function (indx) {
        ...
        .promise()
        .done(function () {
          jQuery(".logo_and_menu-row .menu-only").css({
  ...
  CheckTopMenuOncePadding = function () {
    if (jQuery(".menu-row.sliced .right-icons .wrap_icon").length) {
      ...
      jQuery(".menu-row.sliced .maxwidth-theme>div>div")
        .each(function (indx) {
          if (!jQuery(this).hasClass("menu-only")) {
            var elementPosition = jQuery(this).position().left,
              elementWidth = jQuery(this).outerWidth();
        ...
        .promise()
        .done(function () {
          jQuery(".menu-row.sliced .menu-only").css({
            "padding-left": leftPadding,
            "padding-right": rightPadding,
          });
        });
  ...
  CheckSearchWidth = function () {
    if (jQuery(".logo_and_menu-row .search_wrap").length) {
      var searchPosition = jQuery(".logo_and_menu-row .search_wrap").position()
          .left,
        maxWidth = jQuery(".logo_and_menu-row .maxwidth-theme").width() - 32;
      width = 0;

      jQuery(".logo_and_menu-row .maxwidth-theme>div")
        .each(function () {
        ...
        .promise()
        .done(function () {
          jQuery(".logo_and_menu-row .search_wrap")
            .outerWidth(width)
  ...
  jQuery(document).ready(function () {
    ...
    if (jQuery(".banners-content").length) {
      jQuery(
        ".logo_and_menu-row .logo-row, .eye_version_black_color .menu-row:not(.bgcolored) .menu-only, .eye_version_black_color .menu-row.bgcolored .menu-only"
      ).css("border", "none");
    }
    ...
    if ($mobileMenu.length) {
      ...
      if ($mobileMenu.isLeftSide) {
        ...
        jQuery(document).on("swiperight", function (e) {
          if (
            !jQuery(e.target).closest(".flexslider").length &&
            !jQuery(e.target).closest(".swipeignore").length
          ) {
            OpenMobileMenu();
          }
        });
        ...
        jQuery("#mobileheader").click(function (e) {
          if (
            !jQuery(e.target).closest("#mobilemenu").length &&
            !jQuery(e.target).closest(".burger").length &&
            $mobileMenu.isOpen
          ) {
            CloseMobileMenu();
          }
      ...
      jQuery("#mobilemenu .menu a,#mobilemenu .social-icons a").click(function (
        e
      ) {
        var $this = jQuery(this);
        ...
        if (!$mobileMenu.isOpen) {
          ...
          if ($mobileMenu.isLeftSide) {
            ...
            $mobileMenu.css({
              top:
                +(
                  jQuery("#mobileheader").height() +
                  jQuery("#mobileheader").offset().top
                ) + "px",
            });
    ...
    if (jQuery("#headerfixed .js-nav").length) {
      if (arMedc2Options["THEME"]["HEADER_FIXED"] === 2) CheckHeaderFixedMenu();
    ...
    jQuery("html, body").on("mousedown", function (e) {
      ...
      if (
        !jQuery(e.target).hasClass("inline-search-block") &&
        !jQuery(e.target).hasClass("svg") &&
    ...
    waitingNotExists(
      ...
      function () {
        jQuery(
          ...
          jQuery("#bx-composite-banner .bx-composite-btn").parent().html()
        );
      }
    );
    ...
    jQuery.validator.addMethod(
      ...
      function (value, element, param) {
        ...
        if (
          ...
          jQuery(element).hasClass("required")
        ) {
          status = false;
        } else {
          ...
          if (basket_link.length) {
            ...
            if (jQuery(_this).find("a .count").length) {
              var count = basket_link.find(".count").length
                ? jQuery(_this).find(".count").text()
                : jQuery(_this).find(".count").text();
              basket_link
                .find(".prices")
                .text(jQuery(_this).find(".icon").data("summ"));
              if (basket_link.find(".count").length) {
                ...
                basket_link
                  ...
                  .append(jQuery(_this).find(".count"));
    ...
    jQuery(window).on("onCounterGoals", function (eventdata) {
      if (
        arMedc2Options["THEME"]["YA_GOLAS"] == "Y" &&
        arMedc2Options["THEME"]["YA_COUNTER_ID"]
      ) {
    ...
    if (arMedc2Options["THEME"]["PRINT_BUTTON"] == "Y") {
      setTimeout(function () {
        if (jQuery(".page-top .rss-block.top").length) {
          ...
          ).insertAfter(jQuery(".page-top .rss"));
          ...
          jQuery(
            '<div class="print-link"><i class="svg svg-print"></i></div>'
          ).insertBefore(jQuery(".page-top h1"));
        // else
    ...
    jQuery(document).on("click", ".eye-link", function () {
      if (jQuery(this).hasClass("show_eye_version")) {
        jQuery.cookie("MEDC2_EYE_VERSION_VALUE", "", {
          path: arMedc2Options["SITE_DIR"],
          expires: 365,
        });
      } else {
        jQuery.cookie("MEDC2_EYE_VERSION_VALUE", "Y", {
          path: arMedc2Options["SITE_DIR"],
          expires: 365,
        });
      }
        ...
        jQuery.ajax({
          ...
          success: function (html) {
            if (jQuery(".banners-small.front").length) {
              ...
              jQuery(".bottom_nav").html(
                jQuery(".banners-small .items.row .bottom_nav").html()
              );
              jQuery(".banners-small .items.row .bottom_nav").remove();
            } else {
              jQuery(html).insertBefore(jQuery(".blog .bottom_nav"));
              jQuery(".bottom_nav").html(
                jQuery(".blog .bottom_nav:hidden").html()
              );
        ...
        jQuery.ajax({
          ...
          success: function (html) {
            ...
            if (jQuery(html).find("[detail-pagination]").length) {
              var htmlPagination = jQuery(html)
                .find(
                  jQuery(html)
                    .find("#" + wrapperId + "[detail-pagination]")
                    .data("page-module")
                )
                .html();
              var container = jQuery(html)
                .find("#" + wrapperId + "[detail-pagination]")
                .attr("detail-pagination");
              ...
              var htmlPagination = jQuery(html).find(".pagination_nav").html();
              var contParent = jQuery(html)
                .find("[data-list-parent]")
                .attr("data-list-parent");
              var contCihld = jQuery(html)
                .find("[data-list-child]")
                .attr("data-list-child");
              var $htmlContent = jQuery(
                jQuery("<div>")
                  .append(jQuery(html).find(contCihld + " > div"))
                  .html()
              );
            ...
            var eventdata = { action: "ajaxContentLoaded" };
            jQuery(window).on("onCompleteAction", [
              eventdata,
              jQuery(this).find(".more_text_ajax")[0],
            ]);
            jQuery(window).on("onCompleteActionComponent", [
              eventdata,
              jQuery(this).find(".more_text_ajax")[0],
            ]);
    ...
    jQuery(document).on("click", ".front.sections .ajax_btn .btn", function () {
      ...
      if (!parent.hasClass("loadings")) {
        ...
        jQuery.ajax({
          ...
          type: "POST",
          url:
            arMedc2Options["SITE_DIR"] + "include/mainpage/comp_sections.php",
          success: function (html) {
    ...
    jQuery(".detail .nav-tabs>li a").on("click", function () {
      ...
      if (!$parent.hasClass("active")) {
        $this
          .closest(".tabs")
          .find(".tab-pane .title-tab-heading .btn")
    ...
    jQuery(".bx_filter_input_container input[type=text]").numeric({
      allow: ".",
    });
    ...
    jQuery(document).on("keyup", ".search-input-div input", function (e) {
      ...
      if (jQuery(this).closest("#headerfixed").length) {
        if (e.keyCode === 13) jQuery(".search form").submit();
      }
    });
    ...
    jQuery(".inline-search-show, .inline-search-hide").on(
      "click",
      function (e) {
        if (window.matchMedia("(min-width: 600px)").matches) {
          if (
            typeof jQuery(this).data("type_search") != "undefined" &&
            jQuery(this).data("type_search") == "fixed"
          )
          ...
          if (arMedc2Options["THEME"]["TYPE_SEARCH"] != "fixed") {
            ...
            if (!jQuery("header > .top-block").length) {
              height_block = jQuery(
                ".inline-search-block.fixed.show .search-wrapper"
              ).actual("outerHeight"); //jQuery(this).closest('.maxwidth-theme').actual('outerHeight');
              console.log(height_block);
              if (jQuery(this).closest(".top-block").length)
                height_block = jQuery(this)
                  .closest(".top-block")
                  .actual("outerHeight");
          ...
          if (jQuery(".top-block").length) {
            if (jQuery(".inline-search-block").hasClass("show"))
              jQuery(".inline-search-block").css(
                "background",
                jQuery(".top-block").css("background-color")
              );
            else jQuery(".inline-search-block").css("background", "#fff");
          }
          if (arMedc2Options["THEME"]["TYPE_SEARCH"] == "fixed") {
            if (jQuery(".inline-search-block").hasClass("show"))
              jQuery('<div class="jqmOverlay search"></div>').appendTo("body");
            else jQuery(".jqmOverlay").detach();
          }
        } else location.href = arMedc2Options["SITE_DIR"] + "search/";
      }
    );
    ...
    if (jQuery(".teasers .item").length) jQuery(".teasers .item").sliceHeight();
    if (jQuery(".wrap-portfolio-front .row.items > div").length)
      jQuery(".wrap-portfolio-front .row.items > div").sliceHeight({
        row: ".row.items",
        item: ".item1",
      });
    ...
    jQuery("section.toggle > label").prepend(
      jQuery("<i />").addClass("fa fa-plus")
    );
    jQuery("section.toggle > label").prepend(
      jQuery("<i />").addClass("fa fa-minus")
    );
    jQuery("section.toggle.active > p").addClass("preview-active");
    jQuery("section.toggle.active > div.toggle-content").slideDown(
      350,
      function () {}
    );
    ...
    /* portfolio item */
    jQuery(".item.animated-block").appear(
      function () {
        var $this = jQuery(this);
    ...
    jQuery(".tabs_ajax .head-block .item-link").on("click", function () {
      var index = jQuery(this).index(),
        ...
        url_post =
          arMedc2Options["SITE_DIR"] + "include/mainpage/comp_catalog_ajax.php";
    ...
    jQuery(".thumbs .item a").on("click", function (e) {
      ...
      jQuery(".slides li" + jQuery(this).attr("href"))
        .addClass("current")
        .siblings()
        .removeClass("current");
    });
    ...
    jQuery(document).on("click", ".basket.fly .opener", function () {
      ...
      else jQuery(this).closest(".ajax_basket").toggleClass("opened");
    });
    ...
    jQuery(document).on("click", function () {
      if (
        jQuery(".basket.fly").length &&
        jQuery(".ajax_basket").hasClass("opened")
      ) {
    ...
    jQuery("select.region").on("change", function () {
      ...
      if (jQuery("select.city").length) {
        ...
        } else jQuery("select.city").attr("disabled", "disabled");
      }
    });
    ...
    jQuery(document).on("click", ".to_cart", function (e) {
      ...
      var item = jQuery(this).closest("[data-item]"),
        ...
        countItem = jQuery(".basket").length
          ? parseInt(jQuery(".basket .count").text())
          : parseInt(jQuery(".basket_top:visible .count").text());

      jQuery(".basket_top .count")
        .text(countItem + 1)
        .removeClass("empted");
      jQuery(".basket .count")
        .text(countItem + 1)
      ...
      if (typeof arMedc2Options["THEME"]["ORDER_BASKET_VIEW"] !== "undefined") {
        if (
          jQuery.trim(arMedc2Options["THEME"]["ORDER_BASKET_VIEW"]) ===
            "HEADER" &&
          jQuery(".basket_top").length
        )
          var bBasketTop = true;
        else if (jQuery(".basket.fly").length) var bBasketFly = true;
      }
    ...
    jQuery(document).on("click", ".remove", function () {
      var item = jQuery(this).closest("[data-item]"),
        ...
        bRemoveAll =
          jQuery.trim(
            jQuery(this).closest("[data-remove_all]").data("remove_all")
          ) === "Y"
            ? "Y"
            : false;
      (getCurUri = jQuery.trim(jQuery("input[name=getPageUri]").val())),
        (countItem = jQuery(".basket").length
          ? parseInt(jQuery(".basket .item").length)
          : parseInt(jQuery(".basket_top:visible .item").length)),
        (bOneItem = countItem - 1 <= 0),
        (scrollTop = jQuery(".basket.fly").length
          ? jQuery(".basket.fly .items_wrap").scrollTop()
          : jQuery(".basket_top:visible").length
          ? jQuery(".basket_top .items:visible").scrollTop()
          : "");
        ...
        if (bRemoveAll) {
          ...
          if (jQuery(".basket").length) {
            if (jQuery(".basket_top .count").length)
              jQuery(".basket_top .count").text(
                parseFloat(jQuery(".basket_top:visible .count").text()) - 1
              );
            else {
              jQuery(".basket .count").text(
                parseFloat(jQuery(".basket .count").text()) - 1
              );
              jQuery(".basket_top .count").text(
                parseFloat(jQuery(".basket .count").text()) - 1
              );
          ...
          jQuery("#mobilemenu .menu .ready .count").text(
            parseFloat(jQuery("#mobilemenu .menu .ready .count").text()) - 1
          );
        }
  ...
  scrollPreviewBlock = function () {
    if (
      typeof jQuery.cookie("scoll_block") !== "undefined" &&
      jQuery.cookie("scoll_block")
    ) {
      var scoll_block = jQuery(jQuery.cookie("scoll_block"));
      if (scoll_block.length) {
        jQuery("body, html").animate(
          { scrollTop: scoll_block.offset().top },
          500
        );
  ...
  BasketUpdate = function (el, counterInputValueNew) {
    ...
    if (
      typeof arMedc2Options["THEME"]["ORDER_BASKET_VIEW"] !== "undefined" &&
      jQuery.trim(arMedc2Options["THEME"]["ORDER_BASKET_VIEW"]) === "FLY" &&
      jQuery(".basket.fly").length
    ) {
      var bBasketFly = true;
    }
    ...
    if (
      typeof arMedc2Options["THEME"]["ORDER_BASKET_VIEW"] !== "undefined" &&
      jQuery.trim(arMedc2Options["THEME"]["ORDER_BASKET_VIEW"]) === "HEADER" &&
      jQuery(".basket_top").length
    ) {
      var bBasketTop = true;
    } else {
  ...
  CheckTabActive = function () {
    if (typeof clicked_tab && clicked_tab) {
      if (window.matchMedia("(min-width: 768px)").matches) {
        ...
        jQuery(".catalog.detail .tab-content .tab-pane .title-tab-heading")
          .next()
          .removeAttr("style");
        clicked_tab = 0;
      }
  ...
  jQuery(document).on("onWindowScroll", function (eventdata) {
    try {
      ...
      if (arMedc2Options["THEME"]["TYPE_SEARCH"] !== "fixed") {
        if (
          !jQuery("header > .top-block").length &&
  ...
  jQuery(document).on("onWindowResize", function (eventdata) {
    try {
      ...
      setTimeout(function () {
        createTableCompare(
          jQuery(".main-block .items .title-block:not(.clone) .item"),
          jQuery(".prop_title_table"),
          jQuery(".main-block .prop_title_table .item.clone")
        );
      }, 100);
      SliceHeightBlocks();
    } catch (e) {
        ...
        if (slider) {
          if (slider.closest(".staff-type-2").length) {
            ...
            slider.find(".item").each(function () {
              var deltaSliderHeight = jQuery(this)
                .find(".bottom-block")
                .outerHeight();
        ...
        if (slider) {
          ...
          if (typeof window["players"] == "object" && curSlide.length) {
            ...
            for (var j in window["players"]) {
              var curVideoSlide = jQuery(
                  "iframe[id=" + window["players"][j].id + "]"
                ).closest(".item"),
          ...
          if (
            ...
            jQuery(".gallery-block").closest(".tab-pane").hasClass("active")
          ) {
    ...
    try {
      ...
      if (
        ...
        if (jQuery(eventdata.form).find(".g-recaptcha-response").val()) {
          ...
          grecaptcha.execute(
            jQuery(eventdata.form).find(".g-recaptcha").data("widgetid")
          );
          return false;
        }
      } else {
  ...
  jQuery(
    ".item-views.staff-items.table-type-block .item>.wrap>.wrap"
  ).sliceHeight();

  setTimeout(function () {
    jQuery(
      ".table-type-block.item-views.staff-items.table-type-block .item"
    ).hover(
      function () {
  ...
  jQuery(".accordion").click(function (e) {
    ...
    if (jQuery.inArray(name, headers) > -1) {
      ...
      var allAtDepth = jQuery(".accordion p, .accordion-hide").filter(
        function () {
          if (
            jQuery(this).parents().length >= depth &&
            this !== subItem.get(0)
          ) {
            return true;
          }
  ...
  function TemplateScript() {
    ...
    jQuery(".item-views.staff-items .items .item").hover(
      function () {
        var block = jQuery(this).find(".bottom-block");
        block.show();
        var blockHeight = block.outerHeight(true, true);
